...
```
TIP: you can use the `.flatten` method to turn an array of arrays into a single array.
...
* Here are a few helpful tips:
  ...
  * Look closely at the output string that the test is expecting. You'll notice that it expects holiday names, like "New Years", to have both words capitalized. Ruby has a `.capitalize` method that you can call on a string. **But, note:**
    ...
    * You'll need to capitalize *both words* in a given holiday's name. If you call `"new years".capitalize!`, it will return `"New years"`. In order to capitalize *both* words, you'll need to `.split` the string (at the underscore) into an array and iterate over that array to `.capitalize!` each word in it. Then, you'll need to `.join` the array back into a string.
...
* Write a method to collect all holidays with "BBQ" in the supply array. 
  * Note: you can use the `.compact` array method to remove `nil` elements.
* The method should behave as seen below:
...
**Reminder:** This is a challenging lab, so remember to use Pry, Google, and your classmates to help you get the tests passing.
